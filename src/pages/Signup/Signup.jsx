...
const Signup = () => {
    const { CreateUser, logout } = useContext(AuthContext); // Added `logout` from context
    const navigate = useNavigate();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await CreateUser(email, password); // Create user
                ...
                <div className="card w-1/2 border border-[#D0D0D0] shrink-0">
                    <form onSubmit={handleSubmit} className="card-body">
                        ...
                        <div className='flex flex-col gap-3 items-center'>
                            <p className='text-[#444444] font-medium'>Or Sign Up with</p>
                            <div className='flex flex-row gap-3'>
                                <span className='text-[#3B5998]'><FaFacebook /></span>
                                <span className='text-[#0A66C2]'><FaLinkedin /></span>
                                <FaGoogle />
                            </div>
                            <p>Already have an account? <Link className='text-red-600 font-semibold' to='/login'>Login</Link></p>
                        </div>
